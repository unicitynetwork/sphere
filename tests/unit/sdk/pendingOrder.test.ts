...
describe('pendingOrder', () => {
  ...
  it('round-trips the record for the network it was saved under', () => {
    savePendingOrder('mainnet', record());
    expect(readPendingOrder('mainnet', 1_000_000)).toEqual(record());
  });

  it('does not leak across networks', () => {
    savePendingOrder('mainnet', record());
    expect(readPendingOrder('testnet2', 1_000_000)).toBeNull();
  });
  ...
  it('reads null and self-clears past the 24h recovery horizon', () => {
    savePendingOrder('mainnet', record({ createdAt: 0 }));
    expect(readPendingOrder('mainnet', 24 * 3600_000 + 1)).toBeNull();
    expect(Object.keys(localStorage)).toHaveLength(0);
  });

  it('survives right up to the horizon', () => {
    savePendingOrder('mainnet', record({ createdAt: 0 }));
    expect(readPendingOrder('mainnet', 24 * 3600_000 - 1)).not.toBeNull();
  });
  ...
  it('rejects a corrupt payload instead of returning a half-record', () => {
    ...
    expect(readPendingOrder('mainnet', 1_000_000)).toBeNull();
  });
...
describe('claimPendingOrder', () => {
  ...
  it('lets the first caller take the order and refuses the second', () => {
    savePendingOrder('mainnet', record());
    expect(claimPendingOrder('mainnet', 'ssc-1', 1_000_000)).toBe(true);
    expect(claimPendingOrder('mainnet', 'ssc-1', 1_000_010)).toBe(false);
  });

  it('refuses a claim on a different order than the one stored', () => {
    savePendingOrder('mainnet', record());
    expect(claimPendingOrder('mainnet', 'ssc-other', 1_000_000)).toBe(false);
  });

  it('lets a stale lease be retaken (the holder died)', () => {
    savePendingOrder('mainnet', record());
    expect(claimPendingOrder('mainnet', 'ssc-1', 1_000_000)).toBe(true);
    expect(claimPendingOrder('mainnet', 'ssc-1', 1_000_000 + 120_000)).toBe(true);
  });

  it('refuses when no order is stored', () => {
    expect(claimPendingOrder('mainnet', 'ssc-1', 1_000_000)).toBe(false);
  });
});
