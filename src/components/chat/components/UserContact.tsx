...
export function UserContact({ user, isSelected, onClick }: UserContactProps) {
  const statusColor = user.status === 'online' ? 'bg-emerald-400' : 'bg-neutral-600';
  ...
  return (
    <motion.button
      ...
      className={`w-full p-3 rounded-xl text-left transition-all relative overflow-hidden group ${
        isSelected
          ? 'bg-neutral-800/80 border border-orange-500/50 shadow-lg shadow-orange-500/10'
    ...
    >
      ...
      <div className="flex items-center gap-3 relative z-10">
        <div className="relative">
          ...
          {/* Status dot */}
          <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-neutral-900 ${statusColor}`}>
            {isOnline && (
