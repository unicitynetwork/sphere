        ...
        <div className="flex gap-2">
          {compactActions.map((action) => (
            <button
              ...
              onClick={() => onAction(action.message)}
              disabled={disabled}
              className="w-8 h-8 flex items-center justify-center rounded-lg bg-neutral-100 dark:bg-neutral-800/50 text-neutral-600 dark:text-neutral-400 text-sm hover:bg-neutral-200 dark:hover:bg-neutral-700/50 hover:text-neutral-900 dark:hover:text-white transition-colors disabled:opacity-50"
            >
              {action.label}
            </button>
          ))}
