...
export function WalletPanel() {
  ...
  return (
    <div className="bg-white/60 dark:bg-neutral-900/90 backdrop-blur-xl rounded-3xl border border-neutral-200 dark:border-neutral-800/50 overflow-hidden h-full relative lg:shadow-xl dark:lg:shadow-2xl flex flex-col transition-all duration-500 theme-transition">
      ...
      {/* DYNAMIC CONTENT AREA - Both components stay mounted to preserve state */}
      <div className="flex-1 relative overflow-hidden">
        <motion.div
          animate={{ opacity: activeLayer === 'L1' ? 1 : 0 }}
          transition={{ duration: 0.2 }}
          className="absolute inset-0"
          style={{ pointerEvents: activeLayer === 'L1' ? 'auto' : 'none', zIndex: activeLayer === 'L1' ? 1 : 0 }}
        >
          <L1WalletView showBalances={showBalances} />
        </motion.div>
        <motion.div
          animate={{ opacity: activeLayer === 'L3' ? 1 : 0 }}
          transition={{ duration: 0.2 }}
          className="absolute inset-0"
          style={{ pointerEvents: activeLayer === 'L3' ? 'auto' : 'none', zIndex: activeLayer === 'L3' ? 1 : 0 }}
        >
