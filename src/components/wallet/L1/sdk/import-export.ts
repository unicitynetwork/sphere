...
export async function importWallet(
  ...
  try {
    ...
    // For BIP32 wallets, addresses need to be regenerated by the caller
    // For standard wallets, recover childPrivateKey
    if (!isImportedAlphaWallet && parsedAddresses.length > 0) {
      ...
      // Recover childPrivateKey - exact method from index.html
      ...
      // Generate address from derived key to verify
