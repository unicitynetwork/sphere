/**
 * IPNS Nametag Fetcher (Browser-specific)
 *
 ...
 * Usage:
 * ```typescript
 * import { fetchNametagFromIpns } from '@unicity/wallet-sdk/browser';
 *
 * const result = await fetchNametagFromIpns(privateKeyHex, {
 *   gatewayUrls: ['https://gateway1.example.com', 'https://gateway2.example.com'],
 *   gatewayPathTimeoutMs: 5000,
 *   perGatewayTimeoutMs: 10000,
 * });
 *
 * if (result.nametag) {
 *   console.log('Found nametag:', result.nametag);
 * }
 * ```
 */

import { unmarshalIPNSRecord } from "ipns";
import { deriveIpnsNameFromPrivateKey } from "../../ipfs";

// ==========================================
// Types
// ==========================================

export interface IpnsNametagConfig {
  /** Gateway URLs to query */
  gatewayUrls: string[];
  /** Timeout for gateway path method (fast path) in ms */
  gatewayPathTimeoutMs?: number;
  /** Timeout for routing API method (slow path) per gateway in ms */
...
const DEFAULT_GATEWAY_PATH_TIMEOUT = 5000;
const DEFAULT_PER_GATEWAY_TIMEOUT = 10000;
...
export async function fetchNametagFromIpns(
  ...
  try {
    ...
    const result = await fetchViaHttpGateway(ipnsName, config);
    if (result) {
      return {
        ipnsName,
        nametag: result.name,
...
export async function fetchNametagsForKeys(
  ...
  const promises = privateKeys.map((key) => fetchNametagFromIpns(key, config));
 ...
 */
async function fetchViaHttpGateway(
  ipnsName: string,
  config: IpnsNametagConfig
): Promise<NametagFetchResult | null> {
  const gatewayUrls = config.gatewayUrls;

  if (gatewayUrls.length === 0) {
    throw new Error("No IPFS gateways configured");
  }

  const gatewayPathTimeout = config.gatewayPathTimeoutMs ?? DEFAULT_GATEWAY_PATH_TIMEOUT;
  const perGatewayTimeout = config.perGatewayTimeoutMs ?? DEFAULT_PER_GATEWAY_TIMEOUT;
  ...
  const racePromises = gatewayUrls.flatMap((gatewayUrl) => [
    // Gateway path (fast)
    tryGatewayPath(gatewayUrl, ipnsName, gatewayPathTimeout).catch(() => null),
    // Routing API (slow but reliable)
    tryRoutingApi(gatewayUrl, ipnsName, perGatewayTimeout).catch(() => null),
  ]);
...
async function tryGatewayPath(
  ...
  try {
    contentResponse = await fetchWithTimeout(ipnsUrl, timeoutMs);
  } catch (error) {
...
async function tryRoutingApi(
  ...
  try {
    ...
    const valueStr = typeof record.value === 'string'
      ? record.value
      : new TextDecoder().decode(record.value as Uint8Array);
