...
export class IpfsCache {
  ...
  private contentCache = new Map<string, CacheEntry<unknown>>();
   ...
   */
  getContent<T = unknown>(cid: string): T | null {
    return (this.contentCache.get(cid)?.data as T) || null;
  }

  /**
   * Store immutable content (no expiration)
   */
  setContent(cid: string, content: unknown): void {
