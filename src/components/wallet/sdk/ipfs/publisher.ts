...
export interface IpfsPublisherConfig {
  ...
  /** Timeout for content storage (ms) */
  storeTimeoutMs?: number;
  /** Timeout for IPNS publish (ms) */
...
export class IpfsPublisher {
  private config: Required<IpfsPublisherConfig>;

  constructor(config: IpfsPublisherConfig) {
    this.config = {
      getGatewayUrls: config.getGatewayUrls,
      storeTimeoutMs: config.storeTimeoutMs ?? 3000,
  ...
  async publishTokenData(
    ...
    const lifetime = options.lifetime ?? "87660h";
    const timeoutMs = options.timeoutMs ?? this.config.publishTimeoutMs;

    // Step 1: Store content - return immediately on first success
    const storePromises = gateways.map((gateway) =>
      storeContentOnGateway(tokenData, gateway, this.config.storeTimeoutMs)
        .then((cid) => ({
          result: cid,
          gateway,
          success: cid !== null,
...
/**
 * Create a configured publisher instance
 */
export function createIpfsPublisher(config: IpfsPublisherConfig): IpfsPublisher {
  return new IpfsPublisher(config);
}
